...
def getDictFormattedDatabases(listDatabases):
    ...
    for language_tuples in listDatabases:
        language = language_tuples[0]
        translations = language_tuples[1]
        word_dict = {}
        for word in translations:
            #Set the translation of the word as the value in the dictionary
            word_dict[word[0]] = word[1]
        language_dict[language] = copy.deepcopy(word_dict)
...
def translateFromDict(text, dictBasedDb):
    ...
    for word in words:
        ...
        try:
            ...
            translatedWord = word
        #Add together parts of word and a space
        translation += translatedWord + toAppend + ' '
    return translation.strip()
...
def performDictBasedTranslations(preprocessedText, dictBasedDatabases):
    ...
    """
    for language,translations in dictBasedDatabases.items():
        translatedText = translateFromDict(preprocessedText, translations)
        display(language, translatedText)
